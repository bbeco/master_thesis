...
A sparse point cloud is the product of most SfM pipelines; some keypoints are tracked in each photograph and are then used by the motion estimation algorithm. These keypoints are triangulated and their corresponding world points are available during ego-motion estimation.
%
...
A depth map consists in associating a value of depth for each pixel of the images pair.
...
\emph{normalized cross correlation} (NCC), etc. These provide a way to 
...
Omnidirectional cameras are characterized by a wide field of view. Indeed, many of this kind of devices can take pictures with a 180\degree view angle or even wider.
...
In the previous sections, we described the core steps of a SfM pipeline.
...
FOV cameras that enable the computation of disparity maps with standard 
...
with small motions of the camera as input data.
