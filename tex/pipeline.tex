...
The pipeline has two main parts: an initial camera poses estimation
step, and a densification phase. The former aims to reconstruct the 
camera trajectory, which means to estimate the camera position for each frame.
The latter step relies on the previous result to create a dense 
3D point cloud that represents a reconstruction of the environment.
Fig.~\ref{fig:pipeline_overview} is a plain visualization of these two main parts of the 
pipeline.
...
\textbf{NOTE DI FRANCESCO: due righe su cosa e' un SURF keypoint o un keypoint in generale e erference al paper}
For each new frame  (in the equirectangular format), we locate the
SURF keypoints; we consider those keypoints whose inclination angle is in the range $[-60\degree; +60\degree]$. This is because the poles are affected by great 
