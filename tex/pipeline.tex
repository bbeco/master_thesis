...
by the composition of other transformations. Figure~\todo{aggiungere ref} shows
the sequence of rotations we combine to obtain $R_L$ and $R_R$.

\missingfigure{prendere immagini delle rotationi dal paper. Ricordare che il sistema di riferimento e' diverso}
...
The block-matching family of algorithms compute disparities by extracting the 
...
in the other image. The small squared area extracted from each points is called
...
\begin{itemize}
	\item \emph{patch size}: the number of pixels for each patch's side;
	\item \emph{maximum disparity}: how far, in pixels, the algorithm moves
	above and below the corresponding pixel in the other image (this is the
	horizontal distance for block-matching algorithms for perspective
	photographies);
	\item similarity/error metric employed.
\end{itemize}
...
of pixel $(i,j)$ in the $X$ grey image.
...
SSD(u, v, d) = (1 - \alpha) \left[ \sum_{(k, l)}(I_L(u + k, v + l) - I_R(u + k, v + l + d))^2\right] + \\
	 ...
	 + \left. \sum_{(k, l)}(I_{y,L}(u + k, v + l) - I_{y,R}(u + k, v + l + d))^2 \vphantom{\sum_{(k, l)}} \right]
\end{split}
\end{equation*}
